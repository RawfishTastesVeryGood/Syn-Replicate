	 ...
	 \ \_____  \    \ \    / / \ \  \\ \  \|\____________\ \   _  _\ \  \_|/_\ \   ____\ \  \    \ \  \ \  \    \ \   __  \   \ \  \ \ \  \\\  \ \   _  _\  
	  \|____|\  \    \/  /  /   \ \  \\ \  \|____________|\ \  \\  \\ \  \_|\ \ \  \___|\ \  \____\ \  \ \  \____\ \  \ \  \   \ \  \ \ \  \\\  \ \  \\  \| 
	   ...
	   \|_________\|___|/                                                                                                                                   
		...
		EDITTED VERSION OF SYN-REPLICATED (MADE FOR DoISA)
...
return function(debugMode)
	...
	local function proxyFunc(self, func)
		return function(...)
			local args = {} do
				for index, value in pairs({...}) do
					args[reals(index)] = reals(value)
				end
			end
			return wrapObj(func(self, unpack(args)))
		end
	end

	local function wrapTable(list)
		function wrap(list)
			local t = {}
			for index, value in pairs(list) do
				index, value = reals(index), reals(value)
				if type(value) == "table" then
					t[index] = wrap(value)
				elseif type(value) == "function" then
					t[index] = proxyFunc(value)
				else
					t[index] = value
				end
			end
			...
			CurrentCamera = createFakeObj(workspace.CurrentCamera, {
				...
				__newindex = function(self, index, value)
					if debugMode then
						...
						pcall(function()
							return ReplicateFunction:InvokeClient(realOwner, "Camera", {Index = reals(index), Value = reals(value)})
						end)
					else
	...
	local Holder = Instance.new("ScreenGui", realOwner.PlayerGui)
	...
	local Client = NLS([[
		...
		local serverReplicate = nil do
			repeat
				serverReplicate = script.Parent:FindFirstChildOfClass("RemoteFunction")
				wait()
			until serverReplicate ~= nil
		end
		...
		serverReplicate.OnClientInvoke = function(method, data)
			local object = Methods[method]
			if object then
				object[data.Index] = data.Value
			end
			return true
		end
	...
	ReplicateFunction = Instance.new("RemoteFunction", Holder)
	ReplicateFunction.Name = createRandomID(3)
